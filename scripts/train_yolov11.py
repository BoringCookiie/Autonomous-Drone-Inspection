...
def resolve_dataset_config(data_yaml: Path) -> tuple[dict, Path]:
...
def write_runtime_dataset_yaml(source_yaml: Path, destination: Path) -> Path:
    """Write a portable config with an absolute root for Ultralytics resolution."""
    cfg, base = resolve_dataset_config(source_yaml)
    cfg["path"] = str(base)
...
def main() -> int:
    ...
    try:
        ...
        runtime_data_yaml = write_runtime_dataset_yaml(
            args.data_yaml.resolve(), args.results_dir.resolve() / "resolved_dataset.yaml"
        )
    ...
    model.train(
        data=str(runtime_data_yaml), epochs=args.epochs, batch=args.batch,
    ...
    metrics = YOLO(str(args.output)).val(
        data=str(runtime_data_yaml), split="test", imgsz=args.imgsz,
